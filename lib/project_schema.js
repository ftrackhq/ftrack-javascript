...
function getStatuses(session, projectSchemaId, entityType) {
  ...
  var operations = groupedAttributes.map(function (select) {
    return _operation2.default.query(
      "select " +
        select.join(", ") +
        " from ProjectSchema where id is " +
        projectSchemaId
    );
  });
