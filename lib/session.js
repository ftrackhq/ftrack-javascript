    ...
    _createClass(Session, [{
                ...
                var _ret = function () {
                    if (data.__type__ === 'datetime') {
                        if (_this2.serverInformation && _this2.serverInformation.is_timezone_support_enabled) {
                            ...
                            return {
                                v: _moment2.default.utc(data.value)
                            };
                        }

                        // Return date as local moment object.
                        return {
                            v: (0, _moment2.default)(data.value)
                        };
                    }
            ...
            if (data && data.constructor === Object) {
                var _ret2 = function () {
                    if (data._isAMomentObject) {
                        if (_this3.serverInformation && _this3.serverInformation.is_timezone_support_enabled) {
                            // Ensure that the moment object is in UTC and format
                            // to timezone naive string.
                            return {
                                v: data.utc().format(ENCODE_DATETIME_FORMAT)
                            };
                        }
                        ...
                        // to timezone naive string.
                        return {
                            v: data.local().format(ENCODE_DATETIME_FORMAT)
                        };
                    }

                    var out = {};
                    (0, _forIn2.default)(data, function (value, key) {
                        out[key] = _this3._encode(value);
                    });
        ...
        value: function _mergeCollection(collection, identityMap) {
            ...
            var mergedCollection = collection.map(function (item) {
                ...
                return _this4._mergeEntity(item, identityMap);
            });
        ...
        value: function _mergeEntity(entity, identityMap) {
            ...
            (0, _forIn2.default)(entity, function (value, key) {
                if (value && value.constructor === Array) {
                    mergedEntity[key] = _this5._mergeCollection(value, identityMap);
                } else if (value && value.constructor === Object) {
                    mergedEntity[key] = _this5._mergeEntity(value, identityMap);
                } else {
        ...
        value: function query(expression) {
            ...
            request = request.then(function (responses) {
                ...
                response.data = _this7.merge(response.data);
                return response;
            });
