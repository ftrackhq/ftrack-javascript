...
var Session = /*#__PURE__*/function () {
  ...
  _createClass(Session, [{
    ...
    value: function call(operations) {
      ...
      var url = "".concat(this.serverUrl).concat(this.apiEndpoint); // Delay call until session is initialized if initialization is in
      // progress.
    ...
    value: function ensure(entityType, data) {
      ...
      if (!keys.length) {
        throw new Error("Could not determine any identifying data to check against " + "when ensuring ".concat(entityType, " with data ").concat(data, ". ") + "Identifying keys: ".concat(identifyingKeys));
      }

      var primaryKeys = this.getPrimaryKeyAttributes(entityType);
      var expression = "select ".concat(primaryKeys.join(", "), " from ").concat(entityType, " where");
      var criteria = keys.map(function (identifyingKey) {
        ...
        return "".concat(identifyingKey, " is ").concat(value);
      });
      expression = "".concat(expression, " ").concat(criteria.join(" and "));
      return this.query(expression).then(function (response) {
        ...
        if (response.data.length !== 1) {
          throw new Error("Expected single or no item to be found but got multiple " + "when ensuring ".concat(entityType, " with data ").concat(data, ". ") + "Identifying keys: ".concat(identifyingKeys));
        }
    ...
    value: function getComponentUrl(componentId) {
      ...
      return "".concat(this.serverUrl, "/component/get?").concat((0, _encode_uri_parameters["default"])(params));
    }
    /**
    ...
    value: function thumbnailUrl(componentId) {
      ...
      return "".concat(this.serverUrl, "/component/thumbnail?").concat((0, _encode_uri_parameters["default"])(params));
    }
    /**
     * Create component from *file* and add to server location.
     *
    ...
    value: function createComponent(file) {
      ...
      var request = this.call([{
        ...
        file_name: "".concat(fileName).concat(fileType),
        file_size: fileSize,
